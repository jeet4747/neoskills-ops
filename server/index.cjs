...
app.get('/api/training-calendar', auth(), async (req, res) => {
  try {
      ...
      [nominations, enrollments] = await Promise.all([
        ...
        query(
          `SELECT se.session_id, se.enrollment_id, se.user_id, e.course_name AS enrollment_name,
                  ...
                  u.name AS user_name, s.name AS student_name, s.phone AS student_phone, s.email AS student_email,
    ...
    for (const e of enrollments.rows) {
      ...
      enrollMap[e.session_id].push({ enrollment_id: e.enrollment_id, user_id: e.user_id, enrollment_name: e.enrollment_name, module: e.module, user_name: e.user_name, student_name: e.student_name, student_phone: e.student_phone, student_email: e.student_email, training_fee: e.training_fee, paid_amount: e.paid_amount, pending_amount: e.pending_amount, has_pending_payment: e.has_pending_payment, poc_name: e.poc_name, sales_user_id: e.sales_user_id });
    }
